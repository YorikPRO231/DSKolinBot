...
app.post('/api/bindings', (req, res) => {
    try {
        const { formId, channelId, guildId, formName, pingRoleId } = req.body;
        
        if (!formId || !channelId || !guildId) {
            return res.status(400).json({ error: 'Missing required fields' });
        }

        const binding = bindingsManager.addBinding(formId, channelId, guildId, formName, pingRoleId);
...
app.post('/webhook/form-response', async (req, res) => {
    try {
        ...
        chunks.forEach((chunk, chunkIndex) => {
            ...
            if (chunkIndex === 0 && response.respondentEmail) {
                const cleanEmail = response.respondentEmail
        ...
        let content = '';
        const allowedMentions: any = { parse: [] };
        
        if (binding.pingRoleId) {
            content = `<@&${binding.pingRoleId}>`;
            allowedMentions.roles = [binding.pingRoleId];
        }
...
const PORT = process.env.FORMS_SERVER_PORT || 3000;
...
export function startGoogleFormsServer() {
    app.listen(PORT, () => {
        ...
        if (bindings.length > 0) {
            ...
            bindings.forEach(b => {
                console.log(`  • ${b.formName} → ${b.channelId}${b.pingRoleId ? ' 🔔' : ''}`);
            });
        }
    });
}
