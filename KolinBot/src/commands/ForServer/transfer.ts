...
const transferTable: Record<string, Record<string, { new: number, min: number, max: number }[]>> = {
    "FIB": {
        ...
        "LSPD": [{ new: 2, min: 5, max: 5 }, { new: 4, min: 9, max: 11 }, { new: 5, min: 12, max: 13 }],
        "LSSD": [{ new: 2, min: 4, max: 4 }, { new: 4, min: 7, max: 7 }, { new: 5, min: 8, max: 9 }],
    ...
    "SASPA": {
        ...
        "NG": [{ new: 3, min: 5, max: 6 }, { new: 4, min: 7, max: 7 }, { new: 5, min: 8, max: 8 }, { new: 6, min: 9, max: 9 }, { new: 7, min: 10, max: 10 }, { new: 8, min: 11, max: 12 }, { new: 9, min: 15, max: 15 }],
...
export async function execute(interaction: ChatInputCommandInteraction) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            ...
            `⚠️ Перевод игрока принятого в день блата в течение 7 дней запрещен!`
        );
