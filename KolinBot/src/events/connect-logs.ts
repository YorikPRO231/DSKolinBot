...
export function execute(client: Client) {
    ...
    client.on(Events.GuildMemberRemove, async (member) => {
        try {
            ...
            await logger.logError(client, error as Error, 'GuildMemberRemove (kick check)');
        }
    });

    client.on(Events.ChannelUpdate, async (oldChannel, newChannel) => {
        if ('guild' in oldChannel && oldChannel.guild && 'guild' in newChannel && newChannel.guild) {
            await logger.logChannelUpdate(client, oldChannel as GuildChannel, newChannel as GuildChannel);
        }
    });
}