...
import { getFactionByDiscordId, getFactionByKey, getStateServerIds, getCrimeServerIds } from "../../config/settings-loader";
...
export async function execute(inter: ChatInputCommandInteraction) {
  ...
  if (!guildID) {
    return inter.editReply({
      content: "Ошибка: не удалось определить сервер.",
    });
  }

  const result = getFactionByDiscordId(guildID);
  const currentFactionType = result ? result[0] : "TEST_SERVER";
  const currentFactionData = result
    ? result[1]
    : getFactionByKey("TEST_SERVER")!;
  const isState = currentFactionData.type === "government";

  const stateServerIds = getStateServerIds();
  const crimeServerIds = getCrimeServerIds();
  const serverIds = [...stateServerIds, ...crimeServerIds];
  ...
  const filteredMembers = members.filter((member: GuildMember) => {
    ...
    if (
      member.roles.cache.some((r: any) =>
        /администратор|admin|хелпер|helper|куратор|помощник/i.test(r.name),
      )
    )
      return false;
    return true;
  ...
  await inter.editReply({
    content: `Начинаем проверку ${filteredMembers.size} участников...`,
  });
  ...
  const fetchPromises = serverIds
    ...
    .map(async (sid: string) => {
      ...
      const fResult = getFactionByDiscordId(sid);
      const fData = fResult ? fResult[1] : null;
      const isServerState = fData?.type === "government";
  ...
  await inter.editReply({
    content: `Участники серверов загружены. Начинаем анализ...`,
  });

  const wnFaction = getFactionByKey("WN");
  const mpServerId = getFactionByKey("MP_SERVER")?.discord_id;
  ...
  const wnServer = wnFaction
    ? inter.client.guilds.cache.get(wnFaction.discord_id)
    : null;
  ...
  for (const [memberId] of filteredMembers) {
    ...
    for (const [serverId, serverMembers] of serverMembersMap) {
      if (serverMembers.has(memberId)) {
        const fResult = getFactionByDiscordId(serverId);
        const serverFactionData = fResult ? fResult[1] : null;
        if (!serverFactionData) continue;

        if (serverId === mpServerId) {
          continue;
        }

        if (inter.guildId === wnFaction?.discord_id) {
          const serverMember = await wnServer?.members?.fetch(memberId);
          if (
            serverMember &&
            !serverMember.roles.cache.has(wnFaction!.roles.faction)
          ) {
            continue;
          }
        }

        if (serverId === wnFaction?.discord_id) {
          const wnMember = await wnServer?.members
            .fetch(memberId)
            .catch(() => null);
          if (
            !wnMember ||
            !wnMember.roles.cache.has(wnFaction!.roles.faction)
          ) {
            continue;
          }
        }
  ...
  await inter.editReply({
    content: `Найдено ${totalFound} подозрительных. Отправляю результаты...`,
  });
  ...
  for (let i = 0; i < twinkers.length; i += batchSize) {
    ...
    if (i % 200 === 0 && i > 0) {
      await inter.editReply({
        content: `Отправлено ${i}/${twinkers.length} результатов...`,
      });
    }
  }
  ...
  return inter.editReply({
    content: `Проверка завершена! Найдено: ${totalFound}`,
  });
}