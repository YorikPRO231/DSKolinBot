...
export async function execute(inter: ChatInputCommandInteraction) {
    const adminSurname = await AdminsRepository.getAdminSurname(inter.user.id);
    ...
    try {
        ...
        const embed = new EmbedBuilder()
            .setTitle(`Проверка склада ${faction} | Blackberry`)
            .setDescription(`\`\`\`${parsed[1]}\`\`\``)
...
async function handleWarehouseButtons(
    message: any,
    passport: string,
    userid: string,
...
async function processPunishment(
    ...
    if (punishmentType === 'curator') {
        ...
        for (const item of report.items) {
            if (item.vehicle && !storage.includes('машине')) storage += 'машине, ';
            if (item.family && !storage.includes('семье')) storage += 'семье, ';
            if (item.apartment && !storage.includes('квартире')) storage += 'квартире, ';
            if (item.house && !storage.includes('доме')) storage += 'доме, ';
            if (item.customWarehouse && !storage.includes('арендованном складе')) storage += 'арендованном складе, ';
            if (item.sold && !storage.includes('DarkVito')) storage += 'DarkVito, ';
            if (item.traded && !storage.includes('передано')) storage += 'передано, ';
        ...
        const curatorEmbed = new EmbedBuilder().setColor(0xFFA500).setDescription(curatorText);
        await WarehouseRepository.registerDrain(inter.user.id, passport, config.type, report, '-');
        
        if ('editReply' in inter && typeof inter.editReply === 'function') {
            return inter.editReply({ content: 'Ниже приведен готовый выговор куратора', embeds: [curatorEmbed] });
        } else {
    ...
    await WarehouseRepository.registerDrain(inter.user.id, passport, config.type, report, durationText);
    ...
    await channel.send({
        embeds: [embed],
        content: `Администратор ${adminDisplayName}\nИгрок: ${passport}\nФракция: ${faction}\nНаказание: ${config.name} ${durationText}`
    });
