...
async function getFactionsFromRoles(member: GuildMember, client: Client): Promise<string[]> {
...
async function userHasFactionPermission(member: GuildMember, faction: string, client: Client): Promise<boolean> {
...
async function findLeaders(client: Client, ...factions: string[]): Promise<string> {
...
export async function createTransferRequest(
  ...
  if (logChannel) {
    ...
    await TransfersRepository.pushTransfer(currentRank, currentFrac, targetFrac, passport, interaction.user.id, nickname, 'ожидание', 'ожидание', msg.id);
    if (interaction instanceof StringSelectMenuInteraction) {
...
export async function processApproval(inter: ButtonInteraction, member: GuildMember, data: TransferData, pFaction: string) {
  ...
  if (fromApprove !== 'ожидание' && toApprove !== 'ожидание') {
    ...
    await TransfersRepository.removeTransfer(data.passport);
    if (informStatus) {
      return inter.update({
  ...
  await TransfersRepository.pushTransfer(data.current_rank, data.current, data.destination, data.passport, data.user_id, data.nickname, fromApprove, toApprove, data.msg_id);
  return inter.update({
...
export async function handleTransferButtons(inter: ButtonInteraction, member: GuildMember) {
  ...
  const data = await TransfersRepository.retrieveTransferData(passport);
  if (!data) {
...
export async function handleDenyModal(inter: any, member: GuildMember) {
  ...
  if (authorId) {
    try {
      ...
      await TransfersRepository.removeTransfer(passport);
    } catch (e) {
