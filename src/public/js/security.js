const SecurityModule = (function () {
  ...
  function selectAlertType(element) {
    ...
    document
      .querySelectorAll("#alertTypeDropdown .dropdown-item")
      .forEach((item) => {
  ...
  async function showHistoryByStatic(staticId) {
    if (!staticId) return;
    ...
    title.innerHTML = `<i class="fas fa-history"></i> История жалоб: ${Utils.escapeHtml(staticId)}`;
    content.innerHTML =
      '<div style="text-align: center; padding: 2rem;"><i class="fas fa-spinner fa-spin"></i><p>Загрузка...</p></div>';

    showModal("historyModal");

    try {
      const response = await fetch(
        `/api/security/alerts/history/${encodeURIComponent(staticId)}`,
      );
      const data = await response.json();

      if (data.success && data.logs && data.logs.length > 0) {
        let historyHtml = `<div style="margin-bottom: 1rem; padding: 0.75rem; background: var(--bg-main); border-radius: 8px;"><strong>Всего жалоб:</strong> ${data.logs.length}</div><div class="history-timeline">`;

        for (const log of data.logs) {
          const typeClass = log.type === "Bots" ? "text-bots" : "text-cheats";
          const typeIcon = log.type === "Bots" ? "fa-robot" : "fa-gamepad";
          const typeText = log.type === "Bots" ? "Боты" : "Читы";
          const adminNickname = await getNickname(log.author_id);

          historyHtml += `
                        <div class="history-item">
                            <div class="history-header">
                                <span class="type-badge ${typeClass}"><i class="fas ${typeIcon}"></i> ${typeText}</span>
                                <span class="history-date">${Utils.formatDate(log.created_at)}</span>
                            </div>
                            <div class="history-reason">${Utils.escapeHtml(log.reason)}</div>
                            <div class="history-author"><i class="fas fa-user-shield"></i> Добавил: ${Utils.escapeHtml(adminNickname)}</div>
                        </div>
                    `;
        }
        historyHtml += `</div>`;
        ...
        content.innerHTML =
          '<div style="text-align: center; padding: 2rem;"><i class="fas fa-inbox"></i><p>Нет жалоб на этого игрока</p></div>';
      }
    } catch (error) {
      console.error("Error loading history:", error);
      content.innerHTML =
        '<div style="text-align: center; padding: 2rem; color: var(--danger);"><i class="fas fa-exclamation-triangle"></i><p>Ошибка загрузки истории</p></div>';
    }
  }
  ...
  async function loadAlerts(page = 1) {
    ...
    params.push('sortBy=' + currentSortBy);
    params.push('sortOrder=' + currentSortOrder);
    ...
    for (let i = 0; i < skeletonRows; i++) {
      skeletonHtml += `
                <tr class="skeleton-row">
                    <td class="skeleton-cell"><div class="skeleton skeleton-text" style="width: ${80 + Math.random() * 40}px;"></div></td>
                    <td class="skeleton-cell"><div class="skeleton skeleton-badge" style="width: ${70 + Math.random() * 30}px;"></div></td>
                    <td class="skeleton-cell"><div class="skeleton skeleton-text" style="width: ${120 + Math.random() * 80}px;"></div></td>
                    <td class="skeleton-cell"><div class="skeleton skeleton-text" style="width: ${90 + Math.random() * 50}px;"></div></td>
                    <td class="skeleton-cell" style="text-align: center;"><div class="skeleton skeleton-badge" style="width: 32px; margin: 0 auto;"></div></td>
                    <td class="skeleton-cell"><div class="skeleton skeleton-text" style="width: ${60 + Math.random() * 30}px;"></div></td>
                    <td class="skeleton-cell"><div class="skeleton" style="width: 36px; height: 36px; border-radius: 6px;"></div></td>
                </tr>
            `;
    }
    ...
    try {
      ...
      if (data.success && data.alerts && data.alerts.length > 0) {
        ...
        for (let i = 0; i < data.alerts.length; i++) {
          ...
          html +=
            `<tr class="clickable-row" onclick="if(!event.target.closest('.btn-icon')) SecurityModule.showFullData(${alert.id})">` +
            '<td><strong class="clickable-static" onclick="event.stopPropagation(); SecurityModule.showHistoryByStatic(\'' +
            Utils.escapeHtml(alert.passport) +
            ...
            typeText +
            "</span></td>" +
            '<td><div class="table-text-truncated">' +
            Utils.escapeHtml(shortReason) +
            ' <i class="fas fa-expand" style="font-size: 0.7rem; opacity: 0.5;"></i></div></td>' +
            '<td><span title="ID: ' +
            Utils.escapeHtml(alert.author_id || "-") +
            '">' +
            Utils.escapeHtml(adminNickname) +
            "</span></td>" +
            '<td style="text-align: center;"><span class="count-badge">' +
            (alert.count || 0) +
            "</span></td>" +
            "<td><small>" +
            Utils.formatDate(alert.created_at) +
            "</small></td>" +
            '<td><button class="btn-icon btn-delete" onclick="event.stopPropagation(); SecurityModule.confirmDeleteAlert(' +
            alert.id +
            ')" title="Удалить"><i class="fas fa-trash"></i></button></td>' +
            "</tr>";
        }
        tbody.innerHTML = html;
      } else {
        tbody.innerHTML =
          '<tr><td colspan="7" class="empty-state"><i class="fas fa-folder-open"></i><br>Нет алертов</td></tr>';
        document.getElementById("pagination").style.display = "none";
      }
    } catch (error) {
      console.error("Ошибка:", error);
      tbody.innerHTML =
        '<tr><td colspan="7" class="error-message"><i class="fas fa-exclamation-circle"></i> Ошибка загрузки</td></tr>';
    }
  }
  ...
  async function showFullData(alertId) {
    ...
    try {
      ...
      if (!data.success || !data.alert) {
        detailsContent.innerHTML =
          '<div style="text-align: center;">Ошибка загрузки данных</div>';
  ...
  async function executePendingAction() {
    ...
    try {
      const response = await fetch(
        "/api/security/alerts/" + pendingId + "/close",
        { method: "POST" },
      );
      const data = await response.json();
  ...
  async function copyAllStats() {
    try {
      const response = await fetch(
        "/api/security/alerts" +
          (currentFilterValue !== "ALL" ? "?type=" + currentFilterValue : ""),
      );
      const data = await response.json();
      if (data.success && data.alerts && data.alerts.length > 0) {
